...
/* -----------------------------------------------------------------------------
 ...
 File: es.tid.corba.TIDNamingAdminClient.NewNullDBSpace.java
...
/**
 * new_nulldb_space command. 
 * <p> Copyright 2009 Telef&oacute;nica I+D. Printed in Spain (Europe). 
 * All Rights Reserved.
 * @autor Alvaro Vega Garc&iacute;a
 * @version 1.0
 */

public class NewNullDBSpace {
    
    public static void main(String args[])
    {
        if((args.length == 0) || args[0].equals("-help")){
            System.err.println("Creates a new NamingSpace in server");
            System.err.println("\tin: reads the Agent IOR");
            System.err.println("\tout: the RootContext IOR");      
            System.err.println("Usage: new_nulldb_space -help | (space_name) ");
            return;
        }
        
        if(args.length < 1) { 
            System.err.println("new_nulldb_space: Bad param number");
            System.err.println("Usage: new_nulldb_space -help | (space_name)");
            return;
        }    
        ...
        try {
            ...
            NamingSpaceAccessData space_data = 
                TIDNamingAdminHandler.parse_nulldb_access(args);
            
            
            NamingContext rootContext = naming_agent.new_space(args[0], space_data);
            
            System.out.println(orb.object_to_string(rootContext));
            System.err.println("new_nulldb_space: NamingSpace " + args[0] + " created.");

        } catch (CannotProceed cp) {
            System.err.println("new_nulldb_space: Error creating NamingSpace " + args[0] + 
                               ": " + cp.why);
        } catch (NamingSpaceAlreadyExist ne) {
            System.err.println("new_nulldb_space: NamingSpace " + args[0] + " already exists.");
        } catch (Exception e) {
            e.printStackTrace();	
        }
