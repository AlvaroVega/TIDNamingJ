...
/* -----------------------------------------------------------------------------
 ...
 File: es.tid.corba.TIDNamingAdminClient.UpdateSQLSpace.java
...
/**
 * update_sql_space command. 
 * <p> Copyright 2001 Telef&oacute;nica I+D. Printed in Spain (Europe). All Rights Reserved.
 * @autor Juan A. C&aacute;ceres
 * @version 1.0
 */
 
public class UpdateSQLSpace {
	
    public static void main(String args[])
    {
    if((args.length == 0) || args[0].equals("-help")){
      System.err.println("Updates a NamingSpace in server");
      System.err.println("\tin: reads the Agent IOR");
      System.err.println("Usage: update_sql_space -help | (space_name user password table connection_init");
      return;
    }

    if(args.length < 5) { 
      System.err.println("update_sql_space: Bad param number");
      System.err.println("Usage: update_sql_space -help | (space_name user password table connection_init");
      return;
    }    
    ...
    try {
        ...
        naming_agent.update_space(args[0], space_data);
        
        System.err.println("update_sql_space: NamingSpace " + args[0] + " updated.");
        
    } catch (CannotProceed cp) {
        System.err.println("update_sql_space: Error creating NamingSpace " + args[0] + ": " + cp.why);
    } catch (NamingSpaceNotExist ne) {
        System.err.println("update_sql_space: NamingSpace " + args[0] + " does not exist.");
    } catch (Exception e) {
        e.printStackTrace();	
    }
