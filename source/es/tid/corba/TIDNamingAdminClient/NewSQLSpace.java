...
/* -----------------------------------------------------------------------------
 ...
 File: es.tid.corba.TIDNamingAdminClient.NewSQLSpace.java
...
public class NewSQLSpace {
    ...
    if((args.length == 0) || args[0].equals("-help")){
      ...
      System.err.println("Usage: new_sql_space -help | (space_name user password table connection_init");
      return;
    }
   
    if(args.length < 5) { 
      System.err.println("new_sql_space: Bad param number");
      System.err.println("Usage: new_sql_space -help | (space_name user password table connection_init");
      return;
    }    
    ...
    try {
        ...
        System.err.println("new_sql_space: NamingSpace " + args[0] + " created.");
        
    } catch (CannotProceed cp) {
        System.err.println("new_sql_space: Error creating NamingSpace " + args[0] + ": " + cp.why);
    } catch (NamingSpaceAlreadyExist ne) {
        System.err.println("new_sql_space: NamingSpace " + args[0] + " already exists.");
    } catch (Exception e) {
        e.printStackTrace();	
    }
