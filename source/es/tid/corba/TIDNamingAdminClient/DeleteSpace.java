...
/* -----------------------------------------------------------------------------
 ...
 File: es.tid.corba.TIDNamingAdminClient.DeleteSpace.java
...
public class DeleteSpace {
    ...
    {
    if(args.length < 1) {
      System.err.println("delete_space: Bad param number");
      System.err.println("Usage: delete_space space_name | -help");
      return;
    }
    
    if(args[0].equals("-help")){
      System.err.println("Deletes a NamingSpace in server");
    ...
    try {
        ...
        naming_agent.delete_space(args[0]);
        
        System.err.println("delete_space: NamingSpace " + args[0] + " deleted.");

    } catch (CannotProceed cp) {
      System.err.println("delete_space: Error deleting NamingSpace " + args[0] + ": " + cp.why);
    } catch (NamingSpaceNotExist ne) {
      System.err.println("delete_space: NamingSpace " + args[0] + " does not exist.");
    } catch (Exception e) {
        e.printStackTrace();	
    }
